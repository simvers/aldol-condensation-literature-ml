...
if __name__=="__main__":
    ...
    search_space = {
        'reg__n_estimators': Integer(50, 100),# This is important, which was not there before 
        'reg__max_depth': Integer(3, 5),
        'reg__learning_rate': Real(0.01, 0.3, prior='log-uniform'),
        'reg__subsample': Real(0.6, 0.95),
        'reg__colsample_bytree': Real(0.6, 0.95),          
        'reg__colsample_bylevel': Real(0.6, 1.0),
        'reg__min_child_weight': Integer(10, 20),           # this prevents overfitting
        'reg__reg_alpha': Real(3.0, 10.0),
        'reg__reg_lambda': Real(3.0, 10.0),
        'reg__gamma': Real(0.5, 2.0)
    }

    # cv = RepeatedKFold(n_splits=5, n_repeats=2, random_state=42) ###############NEW############
    cv = KFold(n_splits=10, shuffle=True, random_state=42)

    opt = BayesSearchCV(pipe, search_space, cv=cv, n_iter=200, scoring='r2', random_state=42)
    ...
    path = './data/tmp/'
    model = "xgboost_conservative_10fold"
