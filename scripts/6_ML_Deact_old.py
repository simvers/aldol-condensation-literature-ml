...
# Import data
df = pd.read_csv("data/catalysts/data_deactivation.csv")
...
for model, config in model_config.items():
    if model in model_to_run:
        ...
        opt = GridSearchCV(estimator=pipe, param_grid=config.get('param_grid'), cv=cv, scoring='r2', n_jobs=-1, verbose=2, return_train_score=True)
