"""
Same cross-validated (doi-grouped), engineered-features pipeline as 6_2, but for the
acrylate yield (Y) target instead of STY.

Reads:  data/processed/data_engineered{DATA_TYPE}.csv
Writes: figures/ML_Y/CV_feateng/<model>/*.svg
...
MODEL_TO_TRAIN = ['xgboost_yield', 'lgbm_yield']  # keys from config/ML_models_STY.py's model_config
...
if __name__ == "__main__":
    ...
    for model in MODEL_TO_TRAIN:
        ...
        # Figure directory
        FIGURE_DIR = ROOT / f"figures/ML_Y/CV_feateng/{model}"
        ...
        ml_plotting.plot_model_diagnostics(
            ...
            parity_kwargs={'max_': 1, 'log1p': False, 'target': 'acetate yield /'})
