"""
STY model variant: same cross-validated (doi-grouped) pipeline as 6_2, but using raw
element composition instead of engineered composition features.

Reads:  data/processed/data_engineered{DATA_TYPE}.csv, data/processed/elements.csv
Writes: figures/ML_STY/CV_elem/<model>/*.svg
...
CAT_INPUT = pd.read_csv(ROOT / 'data/processed/elements.csv', header=None).squeeze('columns').to_list()  # raw element composition columns
...
if __name__ == "__main__":
    ...
    for model in MODEL_TO_TRAIN:
        ...
        # Figure directory
        FIGURE_DIR = ROOT / f"figures/ML_STY/CV_elem/{model}"
