...
from functions.ml_models import save_output, load_model_from_tmp, ContinuousStratifiedKFold
from functions.ml_plots import get_learning_curve, plot_learning_curve, plot_cv_distribution, plot_feature_output, plot_partial_dependence
...
# Import data
# data = pd.read_csv("data/catalysts/data_deactivation.csv")
data = pd.read_csv("data/catalysts/data_deactivation_noSi.csv")
...
# Input features
input = [
    'STY_Acryl_mmolhg', # 'Ac_mmolming', 'Fa_mmolming', 
    'MeOH_mmolming', 'Water_mmolming',
    # 'Ac_source', 'Fa_source', 'Stabilizer', 
    'Temperature_K', 'O_content', 'SSA_m2g',
    'av_cov_rad', 'av_n_val',
    'var_cov_rad', 'var_lat_cst', # 'var_n_val',
    # 'var_pca',
] 
output = 'n'
...
print('Data with n: ', len(data))

# Features and target
# x = data.drop(columns=output)
# data.drop(data.loc[data['n'] > 0.5, :].index, axis=0, inplace=True)
x = data[input]
# x['STY_Acryl_mmolhg'] = np.log1p(data['STY_Acryl_mmolhg'])
# x['MeOH_mmolming'] = np.log1p(data['MeOH_mmolming'])
y = data[output]
...
qcut = None
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.15, stratify=qcut, random_state=rs)
...
fig.savefig(f"figures/ML_n/feature_correlation.png", dpi=600, bbox_inches='tight')
...
fig.savefig(f"figures/ML_n/target_correlation.png", dpi=600, bbox_inches='tight')
...
model_to_train = ['xgboost']  #  best_model, xgboost, rf, knn
...
for model in model_to_train:
    ...
    cv = ContinuousStratifiedKFold(n_splits=4, n_iter=50, random_state=rs)
    ...
    min_, max_ = 0, 0.3
    sns.lineplot(x=[min_, max_], y=[min_, max_], ax=ax[1])
    ax[1].text(0.2, 0.8, f"test score: {test_score : .2f}", transform = ax[1].transAxes)
    ax[1].set(xlabel="ln( 1 + Experimental n / mmol h$^{-1}$ g$^{-1}$ )", ylabel="ln( 1 + Predicted n / mmol h$^{-1}$ g$^{-1}$ )",
              xlim=[min_, max_], ylim=[min_, max_], )

    fig.tight_layout()
    fig.savefig(f"figures/ML_n/{model}/{model}_grid_feateng_predictions.png", dpi=600, bbox_inches='tight')
    ...
    fig.tight_layout()
    fig.savefig(f"figures/ML_n/{model}/{model}_grid_feateng_learning_curve.png", dpi=600, bbox_inches='tight')
    ...
    fig.tight_layout()
    fig.savefig(f"figures/ML_n/{model}/{model}_grid_feateng_pfi.png", dpi=600, bbox_inches='tight')
    ...
    fig.savefig(f"figures/ML_n/{model}/{model}_grid_feateng_trainshap.png", dpi=600, bbox_inches='tight')
    fig, ax = plot_partial_dependence(model=ml_model, x=x_test_shap)
    fig.savefig(f"figures/ML_n/{model}/{model}_grid_feateng_testshap.png", dpi=600, bbox_inches='tight')

    fig, ax = plt.subplots()
    shap.plots.beeswarm(shap_values, ax=ax, show=False, plot_size=None, max_display=15)  #, color=plt.get_cmap("red_blue")
    ax.set(xlabel='SHAP value')
    fig.savefig(f"figures/ML_n/{model}/{model}_grid_feateng_beeswarm.png", dpi=600, bbox_inches='tight')
